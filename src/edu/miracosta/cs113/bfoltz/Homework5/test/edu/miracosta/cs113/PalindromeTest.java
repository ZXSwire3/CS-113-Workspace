package edu.miracosta.cs113.bfoltz.Homework5.test.edu.miracosta.cs113;

import static org.junit.Assert.assertTrue;
import static org.junit.Assert.assertFalse;
import static org.junit.Assert.fail;

import edu.miracosta.cs113.bfoltz.Homework5.src.edu.miracosta.cs113.ArrayListStack;
...
public class PalindromeTest {
    ...
    private boolean isPalindrome(String s) {
        ...
        //makes all everything lowercase and removes all spaces
        s = s.toLowerCase().replaceAll("\\s", "");
        ArrayListStack<Character> normal = new ArrayListStack<>();
        ArrayListStack<Character> reverse = new ArrayListStack<>();

        //puts the characters from s into the "normal" stack list
        for (int i = 0; i < s.length(); i++) {
            normal.push(s.charAt(i));
        }

        //puts half the characters from the "normal" stack list into the "reverse" stack list
        for (int i = 0; i <s.length()/2; i++) {
            reverse.push(normal.pop());
        }

        //ignore middle character if there is one
        if (s.length() % 2 != 0) {
            normal.pop();
        }
